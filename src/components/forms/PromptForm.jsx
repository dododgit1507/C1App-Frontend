...
export default function PromptForm({ onSubmit }) {
  ...
  // Función para procesar audio completo con GPT a través del backend
  const procesarAudioConGPT = async (textoCompleto) => {
    ...
    try {
      ...
      // Usar el backend para interpretar la voz
      ...
      if (!areasValidas.includes(resultado.areaNegocio)) {
        resultado.areaNegocio = 'marketing'; // Default fallback
      }
      ...
      // Mostrar éxito
      alert('✅ INTERPRETACIÓN EXITOSA (Backend)\n\n' +
            `📊 Área detectada: ${areasNegocio.find(a => a.id === resultado.areaNegocio)?.nombre}\n` +
            `🎯 Objetivo: ${resultado.objetivo}\n` +
            `⚡ Reto: ${resultado.reto || 'Ninguno especificado'}\n\n` +
            'Los campos se han rellenado automáticamente vía backend. Puedes editarlos si es necesario.');
      
    } catch (error) {
      console.error('Error procesando audio con backend:', error);
      alert('❌ ERROR AL INTERPRETAR (Backend)\n\n' +
            'No se pudo procesar automáticamente tu descripción a través del backend.\n\n' +
            'Posibles causas:\n' +
            '• Backend no está ejecutándose (http://localhost:3001)\n' +
            '• Error de conexión con OpenAI en el backend\n' +
